// src/pages/Generics/AllGenerics.tsx
...
import type { IGeneric } from "../../redux/api/genericApi";

const LIMIT = 10;

const AllGenerics: React.FC = () => {
  ...
  const generics: IGeneric[] = data?.data || [];
  const totalPages = data?.meta?.totalPages ?? 1;
  const totalItems = data?.meta?.total ?? 0;

  const handleDeleteGeneric = async (generic: IGeneric) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete generic "${generic.name}"?`,
        icon: "warning",
      ...
      await deleteGeneric(generic.id).unwrap();
    ...
    <div className="rounded-lg border border-gray-base bg-black-base overflow-hidden">
      <div className="overflow-x-auto">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-4 border-b border-gray-base">
          <div>
            <h1 className="text-xl font-semibold">Generics</h1>
            <p className="text-sm text-gray-400">
              Manage all generic medicines
            </p>
          </div>
              ...
              generics.map((generic, index) => (
                ...
                >
                  ...
                  <td className="py-1 px-4 sm:py-1.5 sm:px-5 border-l border-b border-gray-base">
                    {generic.addedBy ? (
                      <div>
                        <div className="text-sm">{generic.addedBy.name}</div>
                        <div className="text-xs text-gray-500">
                          {generic.addedBy.email}
                        </div>
                      </div>
                    ) : (
                  ...
                  <td className="py-1 px-4 sm:py-1.5 sm:px-5 border-l border-b border-gray-base">
                    <div className="text-xs">
                      <div>
                        {format(new Date(generic.created_at), "MMM d, yyyy")}
                      </div>
                      <div className="text-gray-500">
                        {format(new Date(generic.created_at), "h:mm a")}
                      </div>
                    </div>
                  </td>
                  <td className="py-1 px-4 sm:py-1.5 sm:px-4 border-l border-b border-gray-base">
                    <div className="flex gap-2">
                      <Link to={`/edit-generic/${generic.id}`}>
                        <button
                      ...
                      <button
                        onClick={() => handleDeleteGeneric(generic)}
